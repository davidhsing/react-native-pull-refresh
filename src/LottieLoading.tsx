...
import { useOnPulldownState, usePullRefreshValue } from './hooks';
...
export const HeroLottie = () => {
    ...
    const { pulldownHeight, panTranslateY, pulldownState, containerY, pullingFactor } = ctx;
    ...
    const progress = useDerivedValue(() =>
        [PullingRefreshStatus.BACKUP, PullingRefreshStatus.PULLING].includes(pulldownState.value)
            ? interpolate(actuallyMove(panTranslateY.value, containerY.value), [0, pulldownHeight * pullingFactor], [0, 1], Extrapolate.CLAMP)
            : undefined
    );
